...
def optimization_space(queue, table, channels, samples, flags):
    ...
    for dm in dms_range:
        if flags[0]:
            if flags[1]:
                queue.execute("SELECT local,samplesPerBlock,DMsPerBlock,samplesPerThread,DMsPerThread,GFLOPS FROM " + table + " WHERE (DMs = " + str(dm[0]) + " AND channels = " + channels + " AND samples = " + samples + ")")
            elif flags[2]:
                queue.execute("SELECT local,samplesPerBlock,DMsPerBlock,samplesPerThread,DMsPerThread,GFLOPS FROM " + table + " WHERE (DMs = " + str(dm[0]) + " AND channels = " + channels + " AND samples = " + samples + " AND local = 1)")
            else:
                queue.execute("SELECT local,samplesPerBlock,DMsPerBlock,samplesPerThread,DMsPerThread,GFLOPS FROM " + table + " WHERE (DMs = " + str(dm[0]) + " AND channels = " + channels + " AND samples = " + samples + " AND local = 0)")
            best = queue.fetchall()
            confs.append([best[0][0], best[0][1], best[0][2], best[0][3], best[0][4], best[0][5]])
        else:
            queue.execute("SELECT samplesPerThread,DMsPerThread,GFLOPS FROM " + table + " WHERE (DMs = " + str(dm[0]) + " AND channels = " + channels + " AND samples = " + samples + ")")
            best = queue.fetchall()
            confs.append([best[0][0], best[0][1], best[0][2]])
    return confs
