  ...
  try {
	...
	for ( std::vector< unsigned int >::iterator samples = samplesPerBlock.begin(); samples != samplesPerBlock.end(); ++samples ) {
		for ( std::vector< unsigned int >::iterator DMs = DMsPerBlock.begin(); DMs != DMsPerBlock.end(); ++DMs ) {
          ...
          double gbs = isa::utils::giga(static_cast< long long unsigned int >(observation.getNrDMs()) * ((observation.getNrSamplesPerSecond() * (observation.getNrChannels() + 1)) + observation.getNrChannels()) * sizeof(dataType));
