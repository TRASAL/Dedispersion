  ...
  if ( conf.getLocalMem() ) {
    ...
    if ( (inputDataType == intermediateDataType) && (inputBits >= 8) ) {
      ...
      *code += "buffer[inShMem] = convert_" + intermediateDataType + "(interBuffer);\n";
      ...
      if ( conf.getSplitSeconds() ) {
        ...
        *code += "buffer[inShMem] = convert_" + intermediateDataType + "(input[(" + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels()) - 1) + " * " + isa::utils::toString(observation.getNrSamplesPerDispersedChannel()) + ") + inGlMem]);\n";
      }
    }
    *code += "inShMem += " + nrTotalThreads_s + ";\n"
    ...
    unrolled_sTemplate = "minShift = convert_uint_rtz(shifts[channel + <%UNROLL%>] * (" + firstDM_s + " + ((get_group_id(1) * " + nrTotalDMsPerBlock_s + ") * " + isa::utils::toString(observation.getDMStep()) + "f)));\n"
      ...
      "\n"
      "inShMem = (get_local_id(1) * " + isa::utils::toString(conf.getNrSamplesPerBlock()) + ") + get_local_id(0);\n"
      "inGlMem = sampleOffset + ((get_group_id(0) * " + nrTotalSamplesPerBlock_s + ") + inShMem) + minShift;\n"
      "while ( inShMem < (" + nrTotalSamplesPerBlock_s + " + diffShift) ) {\n";
    if ( (inputDataType == intermediateDataType) && (inputBits >= 8) ) {
      if ( conf.getSplitSeconds() ) {
        unrolled_sTemplate += "buffer[inShMem] = input[(secondOffset * " + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels()) * observation.getNrSamplesPerPaddedSecond()) + ") + ((channel + <%UNROLL%>) * " + isa::utils::toString(observation.getNrSamplesPerPaddedSecond()) + ") + inGlMem];\n";
      } else {
      ...
      if ( conf.getSplitSeconds() ) {
        unrolled_sTemplate += "interbuffer = 0;\n"
          "bitsBuffer = input[(secondOffset * " + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels()) * observation.getNrSamplesPerPaddedSecond() / (8 / inputBits)) + ") + ((channel + <%UNROLL%>) * " + isa::utils::toString(observation.getNrSamplesPerPaddedSecond() / (8 / inputBits)) + ") + (inGlMem / " + isa::utils::toString(8 / inputBits) + ")];\n";
      } else {
        unrolled_sTemplate += "interbuffer = 0;\n"
      ...
      if ( conf.getSplitSeconds() ) {
        unrolled_sTemplate += "buffer[inShMem] = convert_" + intermediateDataType + "(input[(secondOffset * " + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels()) * observation.getNrSamplesPerPaddedSecond()) + ") + ((channel + <%UNROLL%>) * " + isa::utils::toString(observation.getNrSamplesPerPaddedSecond()) + ") + inGlMem]);\n";
      } else {
    ...
    *code += "\n"
    ...
    if ( (inputDataType == intermediateDataType) && (inputBits >= 8) ) {
      if ( conf.getSplitSeconds() ) {
        ...
        sum0_sTemplate = "dedispersedSample<%NUM%>DM<%DM_NUM%> += input[(" + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels() - 1) * observation.getNrSamplesPerDispersedChannel()) + ") + sample + <%OFFSET%>];\n";
      ...
      if ( conf.getSplitSeconds() ) {
        sum0_sTemplate = "interBuffer = 0;\n"
          "bitsBuffer = input[(secondOffset * " + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels()) * observation.getNrSamplesPerPaddedSecond() / (8 / inputBits)) + ") + (" + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels() - 1) * (observation.getNrSamplesPerPaddedSecond() / (8 / inputBits))) + ") + ((sample + <%OFFSET%>) / " + isa::utils::toString(8 / inputBits) + ")];\n";
        sum_sTemplate =  "interBuffer = 0;\n"
          "bitsBuffer = input[((((sampleOffset + sample + <%OFFSET%> + shiftDM<%DM_NUM%>) / " + isa::utils::toString(observation.getNrSamplesPerSecond()) + ") % " + isa::utils::toString(observation.getNrDelaySeconds()) + ") * " + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels()) * observation.getNrSamplesPerPaddedSecond() / (8 / inputBits)) + ") + ((channel + <%UNROLL%>) * " + isa::utils::toString(observation.getNrSamplesPerPaddedSecond() / (8 / inputBits)) + ") + ((sampleOffset + sample + <%OFFSET%> + shiftDM<%DM_NUM%>) % " + isa::utils::toString(observation.getNrSamplesPerSecond() / (8 / inputBits)) + ")];\n";
      } else {
        sum0_sTemplate = "interBuffer = 0;\n"
          "bitsBuffer = input[(" + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels() - 1) * (observation.getNrSamplesPerDispersedChannel() / (8 / inputBits))) + ") + ((sample + <%OFFSET%>) / " + isa::utils::toString(8 / inputBits) + ")];\n";
        sum_sTemplate =  "interBuffer = 0;\n"
      ...
      if ( conf.getSplitSeconds() ) {
        ...
        sum0_sTemplate = "dedispersedSample<%NUM%>DM<%DM_NUM%> += convert_" + intermediateDataType + "(input[(" + isa::utils::toString(static_cast< uint64_t >(observation.getNrChannels() - 1) * observation.getNrSamplesPerDispersedChannel()) + ") + sample + <%OFFSET%>]);\n";
