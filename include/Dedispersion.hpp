...
std::string * getDedispersionOpenCL(const bool localMem, const unsigned int nrSamplesPerBlock, const unsigned int nrDMsPerBlock, const unsigned int nrSamplesPerThread, const unsigned int nrDMsPerThread, const std::string & dataType, const AstroData::Observation & observation, std::vector< int > & shifts);
...
std::string * getDedispersionOpenCL(const bool localMem, const unsigned int nrSamplesPerBlock, const unsigned int nrDMsPerBlock, const unsigned int nrSamplesPerThread, const unsigned int nrDMsPerThread, const std::string & dataType, const AstroData::Observation & observation, std::vector< int > & shifts) {
  ...
  if ( localMem ) {
    *code = "__kernel void dedispersion(__global const " + dataType + " * restrict const input, __global " + dataType + " * restrict const output, __global const int * restrict const shifts) {\n"
      "int dm = (get_group_id(1) * " + nrTotalDMsPerBlock_s + ") + (get_local_id(1) * " + isa::utils::toString< unsigned int >(nrDMsPerThread) + ");\n"
      ...
      "<%DEFS%>"
      "\n"
      "for ( int channel = 0; channel < " + isa::utils::toString< unsigned int >(observation.getNrChannels()) + "; channel++ ) {\n"
    ...
    *code = "__kernel void dedispersion(__global const " + dataType + " * restrict const input, __global " + dataType + " * restrict const output, __global const int * restrict const shifts) {\n"
	...
	std::string shiftsTemplate = "int shiftDM<%DM_NUM%> = shifts[((dm + <%DM_NUM%>) * " + nrPaddedChannels_s + ") + channel];\n";
...
std::string * getDedispersionAVX(const unsigned int nrSamplesPerThread, const unsigned int nrDMsPerThread) {
  ...
  // Begin kernel's template
	*code = "namespace PulsarSearch {\n"
		"template< typename T > void dedispersionAVX" + isa::utils::toString< unsigned int >(nrSamplesPerThread) + "x" + isa::utils::toString< unsigned int >(nrDMsPerThread) + "(const int nrDMs, const int nrSamplesPerSecond, const int nrSamplesPerDispersedSecond, const int nrSamplesPerPaddedSecond, const int nrChannels, const int nrPaddedChannels, const float  * const __restrict__ input, float * const __restrict__ output, const int * const __restrict__ shifts) {\n"
		"#pragma omp parallel for schedule(static)\n"
		"for ( int dm = 0; dm < nrDMs; dm += " + isa::utils::toString< unsigned int >(nrDMsPerThread) + " ) {\n"
			"#pragma omp parallel for schedule(static)\n"
			"for ( int sample = 0; sample < nrSamplesPerSecond; sample += 8 * " + isa::utils::toString< unsigned int >(nrSamplesPerThread) + ") {\n"
				"<%DEFS%>"
				"\n"
...
std::string * getDedispersionPhi(const unsigned int nrSamplesPerThread, const unsigned int nrDMsPerThread) {
  ...
  // Begin kernel's template
	*code = "namespace PulsarSearch {\n"
		...
		"for ( int dm = 0; dm < nrDMs; dm += " + isa::utils::toString< unsigned int >(nrDMsPerThread) + " ) {\n"
			"#pragma omp parallel for schedule(static)\n"
			"for ( int sample = 0; sample < nrSamplesPerSecond; sample += 16 * " + isa::utils::toString< unsigned int >(nrSamplesPerThread) + ") {\n"
				"<%DEFS%>"
				"\n"
